...
# Cursor 設定とテーマの復元スクリプト
# 作成者: キラリ ✨💻

echo "🚀 Cursor 設定とテーマの復元を開始します！"
echo "================================================"
...
if [ -f "$CURSOR_CONFIG_DIR/settings.json" ]; then
    cp "$CURSOR_CONFIG_DIR/settings.json" "${BACKUP_DIR}_original_$(date +%Y%m%d_%H%M%S)/"
fi

if [ -f "$CURSOR_CONFIG_DIR/keybindings.json" ]; then
    cp "$CURSOR_CONFIG_DIR/keybindings.json" "${BACKUP_DIR}_original_$(date +%Y%m%d_%H%M%S)/"
fi

if [ -d "$CURSOR_CONFIG_DIR/snippets" ]; then
    cp -r "$CURSOR_CONFIG_DIR/snippets" "${BACKUP_DIR}_original_$(date +%Y%m%d_%H%M%S)/"
fi
...
if [ -f "$BACKUP_DIR/settings.json" ]; then
    ...
    cp "$BACKUP_DIR/settings.json" "$CURSOR_CONFIG_DIR/"
...
if [ -f "$BACKUP_DIR/keybindings.json" ]; then
    ...
    cp "$BACKUP_DIR/keybindings.json" "$CURSOR_CONFIG_DIR/"
...
if [ -d "$BACKUP_DIR/snippets" ]; then
    echo "📦 snippets/ を復元中..."
    cp -r "$BACKUP_DIR/snippets" "$CURSOR_CONFIG_DIR/"
    echo "✅ snippets/ 復元完了"
else
