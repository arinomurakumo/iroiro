...
# Cursor 設定とテーマのバックアップスクリプト
# 作成者: キラリ ✨💻

echo "🚀 Cursor 設定とテーマのバックアップを開始します！"
echo "================================================"
...
if [ -f "$CURSOR_CONFIG_DIR/settings.json" ]; then
    ...
    cp "$CURSOR_CONFIG_DIR/settings.json" "$BACKUP_DIR/"
...
if [ -f "$CURSOR_CONFIG_DIR/keybindings.json" ]; then
    ...
    cp "$CURSOR_CONFIG_DIR/keybindings.json" "$BACKUP_DIR/"
...
if [ -d "$CURSOR_CONFIG_DIR/snippets" ]; then
    ...
    cp -r "$CURSOR_CONFIG_DIR/snippets" "$BACKUP_DIR/"
...
if [ -f "$CURSOR_CONFIG_DIR/settings.json" ]; then
    # テーマ名を抽出
    THEME_NAME=$(grep -o '"workbench.colorTheme": "[^"]*"' "$CURSOR_CONFIG_DIR/settings.json" | cut -d'"' -f4)
    ICON_THEME=$(grep -o '"workbench.iconTheme": "[^"]*"' "$CURSOR_CONFIG_DIR/settings.json" | cut -d'"' -f4)
